<?php
    ...
    function isActivatedUser($un) {
        global $con;
        $activatedStmt = $con->prepare("SELECT regStatus FROM users WHERE userId = :userId");
        $activatedStmt->bindParam("userId", $un);
        $activatedStmt->execute();
        $regStatus = $activatedStmt->fetchColumn();
        return $regStatus == 1;
    }
    ...
    function goTohttpreferer($alter = "index.php") {
